...
- ✅ 亮色毛玻璃（DWM 亚克力 + 圆角）
- ✅ 自定义分段 Tab + 懒加载
- ✅ 便签：增删改/置顶 + 持久化（`notes.json`，防抖自动保存）
- ✅ 快捷命令：2 列网格 + 添加对话框（多条命令）+ 删除确认 + 执行输出可见 + 持久化
- ✅ 工具：时钟/内存卡片 + exe 启动器网格（添加/浏览/参数）+ 删除确认 + 持久化
- ✅ 系统托盘（显示/收起、开机自启开关、退出）
- ✅ 开机自启（HKCU Run，`StartupService`）
- ✅ 全局热键 Ctrl+Alt+S 切换展开/收起
- ✅ 单实例 Mutex + 关窗不退（仅托盘退出）
- ✅ 编译打包脚本 `build.ps1`（单文件 + zip 到 `dist/`）
- ⏳ 设置页（停靠边/延时/热键自定义）/ 日志 / 多项目拆分 / 单元测试 / 多显示器
...
- [ ] **T1.3 日志**：接入 Serilog，输出到 `%AppData%\SidePeek\logs\`。
- [ ] **T1.4 应用设置**：实现 `ISettingsService` + `AppSettings`，读写 `settings.json`。
- [ ] **T1.5 托盘图标**：托盘菜单（显示/设置/退出），关闭窗口不退出进程。
...
- [ ] **T7.1 设置页**：停靠边、触发延时、主题(深/浅/跟随系统)、开机自启、全局热键。
- [ ] **T7.2 开机自启**：写注册表 `Run` 项，可开关。
- [ ] **T7.3 全局热键**：`RegisterHotKey` 一键唤起/隐藏。
...
- [ ] **T8.2 发布** `[CLI]`
  ```powershell
  dotnet publish src/SidePeek.App -c Release -r win-x64 --self-contained false -p:PublishSingleFile=true
  ```
